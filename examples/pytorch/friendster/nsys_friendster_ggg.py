...
def train(args, device, g, f, sample_mode, batch, workers, pin, prefetch, dataset, model, num_classes, cache_size):
    ...
    train_idx = torch.nonzero(g.ndata['train_mask'], as_tuple=True)[0]
    # train_idx = torch.nonzero(g.ndata['train_mask'], as_tuple=True)[0]
    # temp_f = open('/storage/yash/results/friendster/check_time_2.txt', "a+")
    train_idx = train_idx.to(device)
    num_nodes = g.num_nodes()
    print("Number of nodes: ", num_nodes)
    # access_frequency = {node_id: 0 for node_id in range(num_nodes)}
    ...
    # with open(f'/disk1/access_frequency_files/128_friendster_{sample_mode}_{batch}.csv', 'w', newline='') as file:
    #     writer = csv.writer(file)
    #     writer.writerow(['NodeID', 'AccessCount'])
...
def util(stage):
    ...
    f = open('/media/yash/results/friendster/del_128_1M_gpu_usage_15105.txt', "a+")
...
if __name__ == "__main__":
    ...
    # '/disk1/friendster-dataset/'
    parser.add_argument('--path', type=str, default='/media/yash/dataset/friendster-dataset/', 
    ...
    # f = open('/storage/yash/results/igb_medium/del_gpu_neighbour.txt', "a+")
    # f1 = open('/disk/results/igb_medium/gpu-labor.txt', "a+")
    # f1 = open('/storage/yash/results/igb_medium/del_gpu_labor.txt', "a+")      # Change file
    ...
    # print("Graph data: ", g)
    # print("Node Frames: ", g._node_frames)
    # frames = g._node_frames
    # cache_inf = {"feat": 1000000}
    # for tid, frame in enumerate(frames):
    #     print("TID: ", tid)
    #     print("Graph ntypes: ", g.ntypes)
    #     type_ = [g.ntypes, g.canonical_etypes][0][tid]
    #     print("Frame: ", frame)
    #     for key in frame.keys():
    #         print("Frames keys: ", key)
    #         if key in cache_inf and cache_inf[key] > 0:
    #             column = frame._columns[key]
    #             print("Key", key)
    #             print("Type_", type_)
    #             print("Graph id type", g.idtype)
    #             print("Column: ", column)
    ...
    # f = open("/media/yash/results/friendster_ccc.txt", "a+")
    
    f = open("/media/yash/results/friendster/execTime/Neighbor/pure_ggg_neighbor_15105.txt", "a+")
    if(args.sample_mode == 2 or args.sample_mode == 3):
        f = open("/media/yash/results/friendster/execTime/Labor/pure_ggg_labor_15105.txt", "a+")
