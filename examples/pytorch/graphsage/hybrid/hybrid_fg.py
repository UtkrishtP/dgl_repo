# 2-queue design, with fine grained transfer.
# Here we will start training even if the MFG queue is not full.
...
def ggg(f, g, model, train_idx, sampler_, batch, workers, cache_size,
    ...
    global cpu_samples, epoch, gpu_mfg_queue_2, gpu_mfg_queue_1, queue1_it, queue2_it, queue1_header, queue2_header
    which_queue = 1
    try:
        while epoch > 0:
            # model.train()
            total_loss = tot = tot_wait = 0
            mini_batch = (train_idx.shape[0] + batch - 1) // batch
            # while gpu_buffer_queue.qsize() >= mini_batch and epoch > 0:
            # while (queue1_avail == 1 or queue2_avail == 1) and epoch > 0:
            # while (queue1_header == mini_batch or queue2_header == mini_batch) and epoch > 0:
            while (queue1_header > 0 or queue2_header > 0) and epoch > 0:
                cgg += 1 
                model.train()
                s_ = time.time()
                ...
                if which_queue == 1:
                    queue1_it = 0
                    while mini_batch != queue1_it:
                        w = time.time()
                        # print("Consumed 1")
                        while queue1_it >= queue1_header:
                            time.sleep(1)
                        # print("queue1_it ", queue1_it, queue1_header)
                        w_ = time.time()
                        s = time.time()
                        blocks = gpu_mfg_queue_1[queue1_it]
                        e = time.time()
                    ...
                    queue2_it = 0
                    while mini_batch != queue2_it:
                        w = time.time()
                        # print("Consumed 2")
                        while queue2_it >= queue2_header:
                            time.sleep(1)
                        # print("queue2_it ", queue2_it, queue2_header)
                        w_ = time.time()
                        s = time.time()
                        blocks = gpu_mfg_queue_2[queue2_it]
                        e = time.time()
                        # transfer_mfg_gpu.clear()
                        x = ggg_train_dataloader._cgg_on_demand("feat", "_N", blocks[0].srcdata["_ID"])
                        y = ggg_train_dataloader._cgg_on_demand("label", "_N", blocks[-1].dstdata["_ID"])                
                ...
                mini_batch = (train_idx.shape[0] + batch - 1) // batch
                epoch -= 1
                which_queue = 1 if which_queue == 2 else 2
                f.write("CGG : " + str(time.time() - s_) + " " + str(epoch) + " " + str(time.time()) + " " 
                        + str(tot) + " " + str(tot_wait) + "\n")
                # print("Inside CGG : ", gpu_buffer_queue.qsize(), time.time(), epoch, queue1_it, queue2_it, queue1_header, queue2_header)
...
def transfer_mfg(f, transfer_queue, ggg_launched, done_event, mini_batch,
    ...
    s1 = time.time()
    which_queue = 1
    while not done_event.is_set() or transfer_queue.qsize() > 0 or epoch > 0:
        step = 0
        ...
        # mfgs = []
        # gpu_mfgs = []
        global gpu_mfg_queue_2, gpu_mfg_queue_1, queue1_it, queue2_it, queue1_header, queue2_header
        ...
        if which_queue == 1:
            ...
            # for blocks in mfgs:
                ...
                # To prevent MFG transfer from interferring with GPU sampling we hold all the transfers while the op is happening
                ...
                while True:
                    print("*****")
                    if (queue1_header >= queue1_it or queue1_it == -1) and queue1_header < mini_batch:
                        # print("queue1_header ", queue1_it, queue1_header)
                        # print("*****")
                        gpu_mfg_queue_1[queue1_header] = blocks
                        queue1_header += 1
                        break
                    time.sleep(1)
            ...
            # for blocks in mfgs:
                ...
                while True:
                    # print("$$$$$$")
                    if (queue2_header >= queue2_it or queue2_it == -1) and queue2_header < mini_batch:
                        # print("queue2_header ", queue2_it, queue2_header)
                        # print("$$$$$$$$")
                        gpu_mfg_queue_2[queue2_header] = blocks
                        queue2_header += 1
                        break
                    time.sleep(1)     
        ...
        # print("Inserted MFG ", queue1_it, queue2_it, queue1_header, queue2_header)
...
def process_func(g, model, train_idx, sampler_, batch, workers, cache_size, epoch_, dataset):
    ...
    transfer_queue = Queue(mini_batch * 4) # Queue size has been kept 4 times the mini_batch size. TODO: Make it dynamic
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        mfg_transfer = executor.submit(transfer_mfg, f, transfer_queue, 
                                     ggg_launched, done_event, mini_batch, insert_mfg, insert_mfg_gpu, transfer_mfg_gpu)
        ...
        done_event.set()
        # TODO : Sleep intead of busy-wait
        # while not mfg_transfer.done():
        #     time.sleep(1)
        hybrid_ggg.result()
        mfg_transfer.result()
        
        print("#################### ",  transfer_queue.qsize())
        f.close()
...
if __name__ == "__main__":
    ...
    process_func(g_, model, dataset.train_idx, args.sampler, 
