    ...
    try:
        while epoch > 0:
            ...
            while (queue1_header == mini_batch or queue2_header == mini_batch) and epoch > 0:
                ...
                # if queue1_header == mini_batch:
                #     which_queue = 1
                #     skip = 1

                # if queue2_header == mini_batch:
                #     which_queue = 2
                #     skip = 1

                # if queue1_header > 0 and skip == 0:
                #     which_queue = 1
                
                # if queue2_header > 0 and skip == 0:
                #     which_queue = 2
...
def transfer_mfg(f, transfer_queue, gpu_buffer_queue, ggg_launched, done_event, mini_batch,
    ...
    while not done_event.is_set() or transfer_queue.qsize() > 0:
        ...
        s_ = time.time()
        # print("Insert MFG")
        # At this stage we have transferred 1 mini-batch worth of MFG's to GPU.
            
        if queue1_it == -1:
            print("first touch q1")
            which_queue = 1
        elif queue2_it == -1:
            print("first touch q2")
            which_queue = 2
