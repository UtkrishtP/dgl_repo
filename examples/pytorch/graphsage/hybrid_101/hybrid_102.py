from imports import *
# breakdown_1 numbers for basic hybrid and accuracy check
...
def training_worker(sampler_, size, fanout, train_, model, batch_size, mini_batch, 
    ...
    file = open("../results/hybrid_breakdown_1.txt", "a")
        ...
        while tail.value < head.value:
            ...
            while head_gpu.value - tail_gpu.value >= (mini_batch - 4):
                if sampling.is_set():
                    break
                continue
            gpu_consumer_wait_time = time.time() - start   
            if sampling.is_set():
                break
            ...
            while not free_space(free_space_hbm, sizes, free_mfg_hbm):
                continue
            free_space_wait_time += time.time() - start
