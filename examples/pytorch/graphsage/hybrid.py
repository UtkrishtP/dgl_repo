...
def train(args, device, g, dataset, model, num_classes, f, use_uva, _pin, _alternate, _prefetch_th):
    ...
    gpu_sampler = NeighborSampler(
        [10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
        prefetch_node_feats=["feat"],
        prefetch_labels=["label"],
    )

    cpu_sampler = NeighborSampler(
        [10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
        prefetch_node_feats=["feat"],
        prefetch_labels=["label"],
    )

    cpu_sampler_ = NeighborSampler(
        [10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
        # prefetch_node_feats=["feat"],
        # prefetch_labels=["label"],
    )
    ...
    # Starting CPU sampling in a new thread, to overlap with the GPU training pipeline
    ...
    # print(end - start)
    print("GPU UVA sampling: ", end - start - end1)
    ...
    # print(end - start)
    print("CPU MP sampling : ", end - start - end1)
    ...
    # end = timer()
    # # print(end - start)
    # print("Hybrid : ", end - start - count - count1)
    # global prof
    # prof = 1
    # th.join()
    # print("CPU samples : ", cpu_samples, cpu_samples_processed)
    ...
    # cpu_samples = 0
    # th = threading.Thread(target=cpu_sample, args=(cpu_dataloader_,))
    # th.start()
...
if __name__ == "__main__":
    ...
    f = open('/disk/results/pipeline-mp/arxiv.txt', "a")
    ...
    for batch in reversed(b):
        print("Batch : ", batch)
        train(args, device, g, dataset, model, num_classes, f, args.uva, args.pin_prefetcher, args.alternate_streams, args.prefetch_thread)
