...
def deque_cpu_mfgs(
    ...
    start = time.time()
    while (
        head_gpu.value - tail_gpu.value >= (mfg_buffer_size - 4)
...
def main_worker(file, args, model, train_idx, val_idx, test_idx, g, timestamp):
    ...
    cpu_shared_queue = torch.multiprocessing.Queue(maxsize=mini_batch * 1)
    ...
    train_.set()
    # file.write(f"average valid accu: {np.mean(valid_accum):.4f} ± {np.std(valid_accum):.4f}\n")
...
if __name__ == "__main__":
    ...
    if args.dataset.startswith("igb"):
        ...
        if args.dataset_size == "full" or args.dataset_size == "large":
            args.nfeat_dim = 128
            args.cache_size = 80449000 if args.sampler != "shadow" else 70449000
        else:
            args.nfeat_dim = 1024
    # else:
    #     args.nfeat_dim = 128
