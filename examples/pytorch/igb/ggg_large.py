...
def train(args, device, g, f, sample_mode, batch, workers, pin, prefetch, dataset, model, num_classes, cache_size):
    ...
    train_idx = torch.nonzero(g.ndata['train_mask'], as_tuple=True)[0]
    # train_idx = torch.nonzero(g.ndata['train_mask'], as_tuple=True)[0]
    # temp_f = open('/storage/yash/results/igb_medium/check_time_2.txt', "a+")
    train_idx = train_idx.to(device)
    num_nodes = g.num_nodes()
    print("Number of nodes: ", num_nodes)
    # access_frequency = {node_id: 0 for node_id in range(num_nodes)}
    ...
    if sample_mode == 0:
        sampler = NeighborSampler(
            [15, 10],  # fanout for [layer-0, layer-1, layer-2]
            # fused = False,
            prefetch_node_feats=["feat"],
            prefetch_labels=["label"],
        )
...
def util(stage):
    ...
    f = open('/media/yash/results/igb_large/del_gpu_usage_1510.txt', "a+")
...
if __name__ == "__main__":
    ...
    f = open("/media/yash/results/igb_large_batchfan.txt", "a+")
