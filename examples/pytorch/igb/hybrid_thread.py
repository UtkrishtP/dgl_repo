    ...
    while not done_event.is_set() or transfer_queue.qsize() > 0:
        ...
        if which_queue == 1:
            ...
            # for blocks in mfgs:
                ...
                if queue1_header < queue1_it or queue1_it == -1:
                    # print("Append")
                    gpu_mfg_queue_1[queue1_header] = blocks
                    queue1_header += 1
                else:
                    while queue1_header >= queue1_it:
                        time.sleep(1)
                    gpu_mfg_queue_1[queue1_header] = blocks
                    queue1_header += 1
        else:
            ...
            # for blocks in mfgs:
                ...
                e = time.time()
                tr += e - s
                if queue2_header < queue2_it or queue2_it == -1:
                    # print("Append")
                    gpu_mfg_queue_2[queue2_header] = blocks
                    queue2_header += 1
                else:
                    while queue2_header >= queue2_it:
                        time.sleep(1)
                    gpu_mfg_queue_2[queue2_header] = blocks
                    queue2_header += 1     

        if which_queue == 1:
            # queue1_it = 0 if queue1_it == -1 else queue1_it
            queue1_it = 0
            which_queue = 2
        else:
            # queue2_it = 0 if queue2_it == -1 else queue2_it
            queue2_it = 0
            which_queue = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, default='/data/igb_large/', 
    ...
    # exit(0)
    out_size = 19
    model = SAGE(in_size, 128, out_size)
    ...
    process_func(g, model, train_idx, args.sampler, 
                 args.batch_size, args.workers, args.cache_size, args.epoch, "IGB-large")
