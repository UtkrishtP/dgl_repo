...
class IGBHeteroDGLDataset(DGLDataset):
    ...
    def process(self):
        ...
        # print(self.graph.num_nodes('paper'))
        if self.args.in_memory:
            # paper_node_features = torch.from_numpy(np.load(osp.join(self.dir, self.args.dataset_size, 'processed', 
            # 'paper', 'node_feat.npy')))
            # print("Paper node features", paper_node_features, paper_node_features.shape)
            paper_node_features = np.random.rand(self.graph.num_nodes('paper'), 128).astype('f')
            # Convert the NumPy array to a PyTorch tensor
            paper_node_features = torch.from_numpy(paper_node_features)
        ...
        if self.args.in_memory:
            ...
            author_node_features = np.random.rand(self.graph.num_nodes('author'), 128).astype('f')
            author_node_features = torch.from_numpy(author_node_features)
        else:
            author_node_features = torch.from_numpy(np.load(osp.join(self.dir, self.args.dataset_size, 'processed', 
        ...
        if self.args.in_memory:
            ...
            institute_node_features = np.random.rand(self.graph.num_nodes('institute'), 128).astype('f')      
            institute_node_features = torch.from_numpy(institute_node_features)
        else:
            institute_node_features = torch.from_numpy(np.load(osp.join(self.dir, self.args.dataset_size, 'processed', 
        ...
        if self.args.in_memory:
            ...
            # 'fos', 'node_feat.npy'))) 
            fos_node_features = np.random.rand(self.graph.num_nodes('fos'), 128).astype('f')      
            fos_node_features = torch.from_numpy(fos_node_features)
        else:
            fos_node_features = torch.from_numpy(np.load(osp.join(self.dir, self.args.dataset_size, 'processed', 
        ...
        print("Different node features shapes")
        print(paper_node_features.shape)
        print(author_node_features.shape)
        print(institute_node_features.shape)
        print(fos_node_features.shape)
