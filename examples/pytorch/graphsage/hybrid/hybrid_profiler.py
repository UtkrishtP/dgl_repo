    ...
    try:
        ...
        ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True) as prof:
            while epoch > 3:
                ...
                # while gpu_buffer_queue.qsize() >= mini_batch and epoch > 0:
                # while (queue1_avail == 1 or queue2_avail == 1) and epoch > 0:
                while (queue1_header == mini_batch or queue2_header == mini_batch) and epoch > 3:
                    cgg += 1 
...
def process_func(g, model, train_idx, sampler_, batch, workers, cache_size, epoch_, dataset):
    ...
    transfer_queue = Queue(mini_batch * epoch_) # Queue size has been kept 4 times the mini_batch size. TODO: Make it dynamic
