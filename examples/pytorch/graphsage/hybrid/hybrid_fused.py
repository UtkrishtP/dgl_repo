...
def train(args, device, g, dataset, model, num_classes, f, batch_size, use_uva, _pin, _alternate, _prefetch_th, epoch_):
    ...
    cpu_fused_sampler_prefetch = NeighborSampler(
        [10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
        prefetch_node_feats=["feat"],
        prefetch_labels=["label"],
    )
    ...
    hybrid(cpu_fused_dataloader, gpu_neighbor_dataloader, epoch_, 0)
    hybrid(cpu_fused_dataloader_prefetch_mfg, gpu_neighbor_dataloader, epoch_, 1)
    hybrid(cpu_fused_dataloader_prefetch_nfeats, gpu_neighbor_dataloader, epoch_, 0)
    hybrid(cpu_fused_dataloader_prefetch_nfeats_mfg, gpu_neighbor_dataloader, epoch_, 1)


if __name__ == "__main__":
    # f = open('/mnt/utk/data/dgl-latest/sage_node_class_product.txt', "a")
    f = open('/disk/results/hybrid/fused.txt', "a")
